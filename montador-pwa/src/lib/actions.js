...
export const actions = {
  ...
  async finalizar_assistencia({ osId, tenantId, userId, motivo, observacoes, lat, lng }) {
    const agora = new Date();
    const { error: e1 } = await supabase.from("ordens_servico").update({
    ...
    if (e1) throw e1;
    const { error: e2 } = await supabase.from("os_eventos").insert([{ tenant_id: tenantId, os_id: osId, user_id: userId, tipo: "checkout", latitude: lat, longitude: lng, metadata: { motivo } }]);
    if (e2) throw e2;
  },
